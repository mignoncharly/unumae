...
| 12 | ✅ **Supabase plan decided: staying on Free** | Pro is no longer a gate. Nothing in the schema, Edge Functions or app depends on it, so upgrading later is a billing change rather than a migration. The self-managed encrypted backup covers the missing managed backups; `hosted-health.yml` covers inactivity pausing; one-day log retention is accepted, with incident evidence coming from `operational_alerts` and `job_runs`, which are inside the backup. See `docs/BACKUP_RESTORE.md`. | Nothing. |
