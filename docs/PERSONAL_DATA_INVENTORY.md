...
| `abuse_rate_limits` | None by schema | HMACed scope key, bounded window/count | Server-only queue protection | Two days; only active rate windows are operationally useful | Not personal by contract; opaque keys and thresholds withheld | Purged daily |
| `analytics_events` | `user_id` or attested session UUID in `install_id` | event, bounded properties, day/time; redacted crash name/message/stack/runtime for `client_crash` | Internal aggregate product measurement and first-party crash diagnosis; never authoritative for product state | 90 days; measure and diagnose beta without behavioral profiles | `analytics_events` for signed-in subject | Account deletion removes the user link; de-identified rows and identifier-free website rows expire after 90 days |
...
| `founding_moderators` | email | legacy normalized staff bootstrap email, note/time | Deprecated; Phase 10 empties the table and operator tooling grants roles after signup | Empty; retained only for migration compatibility | Withheld from automatic user export | Phase 10 truncates existing bootstrap rows |
...
| Supabase Vault scheduler secrets | None | encrypted functions URL and service credential | Service-only scheduled-job dispatch | Until rotated | Always withheld | Replaced through service-role-only tooling; no public table remains |
| `installation_sessions` | `user_id` | attestation link, token hash, expiry/use/revocation times | Server-only proof that an app installation passed platform attestation | 30 days, then purged after one-day grace | `installation_sessions`; bearer token hash withheld | Cascade with profile or attestation |
| `expo_push_receipts` | `user_id`; raw token while pending | Expo ticket, push token, lease/retry state, bounded provider category | Confirm push delivery and disable invalid destinations | Until receipt succeeds, destination is disabled, or dead letter is reviewed | Internal delivery state withheld; delivery outcome appears in `notification_deliveries` | Cascade with profile/token; successful receipts removed |
| `translation_failures` | Content target UUID | target field/locale, attempts, bounded provider category | Retry translation without retaining content in error logs | Until success or dead-letter review | Internal operational state withheld | Success removes row; dead letters retained for review |
| `resource_quota_status` | None | configured limits, aggregate storage/egress usage, sample time | Alert before service quota degradation | Latest singleton observation | Not personal; not exported | Replaced by each observation |
...
| `notification_deliveries` | `user_id` | category/channel, destination hash, provider/error/status/time | Server-only delivery diagnostics | 90 days | `notification_deliveries`; destination hash/provider ID withheld | Cascade with profile or daily purge |
| `notification_log` | `user_id` | category, dedupe key, sent time | Private delivery history/idempotency | 365 days | `notifications_sent` | Cascade with profile or daily purge |
...
| `portrait_element_revisions` | Via portrait owner | prompt key and monotonic revision only; no answer text | Server-only stale-write protection | With draft/source portrait | Withheld implementation metadata; the answer itself is exported in `portraits.responses` | Cascade with portrait |
...
| `push_tokens` | `user_id` | token, platform, attested installation, seen times | Server-only push delivery | Until sign-out, invalidation, session replacement, or account deletion | Platform/times in `registered_devices`; raw token withheld | Cascade/sign-out cleanup; session link cleared on expiry |
...
- Export key: `installation_sessions`
