...
| `analytics_events` | `user_id` or attested session UUID in `install_id` | event, bounded properties, day/time | Internal aggregate product measurement; never authoritative for product state | 90 days; measure beta without behavioral profiles | `analytics_events` for signed-in subject | Cascade for account-linked rows; identifier-free website rows expire after 90 days |
